...
One file we'll be using today is the *Album Sales* file. You can either import the file in R Studio by downloading it first or you can actually import the file from the website itself. So instead of importing the file on you computer you simply point R Studio to the web address of the file. To make things easier, that's how we will do it here.

Here is the code for importing the file from the web.

```{r, eval=TRUE}
library(haven)
Album_Sales <- read_sav("https://github.com/jvanster/Psy_300_Datasets/raw/refs/heads/main/SPSS_Datasets/Album%20Sales.sav")
View(Album_Sales)
```

## Descriptive Statistics

Descriptive statistics are just what the name implies, they are used to *describe* a dataset. This is different from *inferential* statistics, which are used to *infer* population parameters based on a sample.
...
One measure of central tendency is the *mean or average*, which shows us the center of a distribution of data. The formula for the mean is very straightforward. It's simply the sum of all the scores divided by the number of scores. The symbol for the sample mean is an x with a bar over it.
...
Often times we're interested in the average spread or deviation from the mean. *Deviance* is the distance of any particular score from the mean. So to find the deviance you simply take the score and subtract it from the mean.
$$deviance = \bar x - x$$
...
Unfortunately this equation causes some problems. All the positive values above the mean and negative values below the mean cancel each other out, leaving a total deviance of zero. Zero doesn't tell us much about variability and deviation, so we need a way to fix this issue. 

The way to overcome this problem is by squaring each deviation score, which makes all the deviation values positive and thus produces a positive number. This number is called the *sum of squared errors* or the *sum of squares* (SS) with the following formula.

$$sum\;of\;squares (SS) = \Sigma(X-\bar X)^2$$

This number is still somewhat inflated, since it's constructed based on squared values. One way to fix this issue to find the average dispersion, which will be based on the number in our sample. Since the sample is an estimate of the population, we actually don't use N, but N -1. This number is called *variance* and has the following symbol and equation.
...
Another measure of dispersion is range. This is simply subtracting the lowest score from the highest score for a particular variable or vector of scores. So to find the range simply use the `range` function and then subtract the two numbers provided, which are the min and max.
...
Another helpful type of range is the *interquartile range*, which is the range of numbers from the 25th and 75th percentile. Percentiles are just dividing up a dataset based on where certain scores are based on percentages. For example, we can look at what score is at the 50th percentile. To do that we use the `quantile` function. x is the variable we are analyzing, and .5 is the percentile (.5 = 50%; .35 = 35%)

```{r, eval=TRUE}
quantile(x = diamonds$carat, probs = .5)
```

So the number at the 50th percentile is 0.7. Notice that this is the same as the median, described earlier. Remember that the median is the number from the dataset that is in the middle or center of the dataset. Because the mean is calculated, it may or may not be a number in the dataset.

So to find the interquartile range, we want our two quartiles or numbers at "quarters" of the dataset, so 25% and 75%. Think of a dollar, which is made up of 4 quarters. We already know that half a dollar is 50 cents, which would be the median, but the other two quartiles (quarters) would be 25 cents and 75 cents.

```{r, eval=TRUE}
quantile(x = diamonds$carat, probs = c(.25, .75))
```

Finally, to get the interquartile range we simply subtract these two numbers.
